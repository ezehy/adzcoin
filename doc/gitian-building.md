...
- [Building Proton Core](#building-adzcoin-core)
...
git clone https://github.com/adzcoincore/adzcoincoin
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/adzcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/adzcoincore/adzcoincoin
...
For example:
```bash
URL=https://github.com/crowning-/adzcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit adzcoin=${COMMIT} --url adzcoin=${URL} ../adzcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit adzcoin=${COMMIT} --url adzcoin=${URL} ../adzcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit adzcoin=${COMMIT} --url adzcoin=${URL} ../adzcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the adzcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../adzcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/adzcoincore/adzcoincoin-detached-sigs.git

BTCPATH=/some/root/path/adzcoincoin.git
SIGPATH=/some/root/path/adzcoincoin-detached-sigs.git

./bin/gbuild --url adzcoin=${BTCPATH},signature=${SIGPATH} ../adzcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/adzcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/adzcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/adzcoin-osx-build.assert
```
...
[adzcoin/gitian.sigs](https://github.com/adzcoincoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
